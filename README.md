...
- Лёгкий ML-пайплайн на чистом Python с признаками временных рядов.
...
- В примере заложен прогноз по рыночным данным BTC/USD и fallback-эвристика, если модель ещё не обучена.
- Для production лучше подключить очереди задач для регулярного переобучения, мониторинг дрейфа и отдельный процесс фоновой агрегации маркет-данных.
